...
$(document).ready(function(){
  ...
  $(function(){
    ...
    latestDeliveryPrice = $('.formfastbuttons .changeprice').text().toString().replace(/\s/g, '');
    TotalSumDelivery = $('.formfastbuttons .TotalSumDelivery .price .num').text().toString().replace(/\s/g, '');
    currentPriceWithoutChange = $('.cartSum:eq(0) .num').text().toString().replace(/\s/g, '');
  ...
  $('.deliveryZoneRadio').click(function(){
    ...
    var priceBlock = $('.deliveryOption[rel='+ val +']').find('.delivery__price').find('.num');
    // Обновление цены
    ...
    $('.formfastbuttons .cartSumTotal .num').text(newSum);
  ...
  $(function(){
    $('.orderStageDeliveryListTable').on('change','.deliveryRadio',function(){
      ...
      var id = $(this).val();
      var fz = $($('.deliveryZoneRadio[deliveryid='+id+']')[0]);       
      $(this).prop('checked',true);
      fz.prop('checked',true);
      var price = $(this).attr('price');
      var priceBlock = $('.deliveryOption[rel='+ id +']').find('.delivery__price').find('.num');
      ...
      // Обновление цены с учетом доставки
      var cartSumTotalHide = $('.cartSumTotalHide:eq(0) .num').text().toString().replace(/\s/g, '');
      var newSum = parseInt(cartSumTotalHide) + parseInt(priceBlock.text().toString().replace(/\s/g, ''));
      $('.formfastbuttons .cartSumTotal .num').text(newSum);
    });
  });
...
function ajaxnewqty(){
  $('.cartqty').change($.debounce(300, function(){
    ...
    if(qVal >= '1'){
      ...
      $('.cart__item[data-id="' + id + '"] .ajaxtotal').css('opacity','0');
      ...
      $.ajax({
        ...
        success:function(d){        
          ...
          $('.cart__item[data-id="' + id + '"] .ajaxtotal').css('opacity','1');
          ...
          c = $(d).find('.cart__item[data-id="' + id + '"] .cartqty');
          qw = c.val();
          if(Number(qVal) > Number(qw)){
            $('.cartErr').remove();
